    ...
    def _is_critical_value(self, result: Dict) -> bool:
        ...
        if test_code in self.critical_values:
            try:
                ...
                if is_critical:
                    logger.warning(f"Critical value: {test_code} = {value} (limits: {limits['low']}-{limits['high']})")
                return is_critical
            except ValueError:
                return False
        return False
